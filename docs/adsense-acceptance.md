...
- [ ] **打开 `public/privacy.html`，确认包含以下条款**：使用 Cookie；第三方供应商（含 Google）使用 Cookie 投放个性化广告；用户可通过 Google Ads Settings 退出；（有欧盟流量则）同意机制。这是 AdSense 隐私政策的**硬性要求**。
- [ ] 在 `index.html` 的 `<head>` 放入 AdSense 验证 snippet（注册后 Google 提供）。
- [ ] 将 sitemap 提交到 **Google Search Console**，确认页面已被索引（被索引的页面越多，审核越顺）。
- [ ] 根域也放一份 `ads.txt`（`magikaru.com/ads.txt`），覆盖子域爬虫的不确定行为。
- [ ] 确认 About / Contact 页有**真实可达的联系邮箱**，不要只有表单。
- [ ] 自查导航：从首页能点到 Privacy / About / Contact（页脚链接即可）。
- [ ] 移动端 + Lighthouse 跑一遍，确认无明显体验问题（弹窗、错位、加载失败）。
- [ ]（可选）补一个独立 Terms 页提升可信度。
