...
## 持久化 Persistence（Supabase Postgres）

三张表：`repos`（按 `full_name` 去重）、`snapshots`（每天 × 周期 × 语言唯一）、`rankings`。
读取走 `server/store.js`：命中 30 分钟内的 DB 快照直接返回，过期则重新抓取入库；上游故障时回退最近快照。**未配置 Supabase 时自动回退为实时抓取 + 内存缓存**（无历史）。

### 配置 Setup

1. 在 [supabase.com](https://supabase.com) 建项目；SQL Editor 里执行 `supabase/schema.sql`。
2. 复制 `.env.example` 为 `.env`，填入 `SUPABASE_URL` 与 `SUPABASE_SERVICE_ROLE_KEY`（Project Settings → API）。
3. `npm run ingest` 抓一次写入今天的快照（daily/weekly/monthly）。

> `server/store.js`、`ingest.js`、`scrapeTrending.js` 与运行时无关，后续接 Cloudflare 时整体搬进 Worker（cron 定时 ingest + 静态托管），前端不动。
