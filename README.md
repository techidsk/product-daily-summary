...
- **`index.html`**：完整 `<head>`——title / description / keywords / canonical / hreflang / Open Graph / Twitter Card / `theme-color` / 图标 / PWA manifest，外加 `WebSite`、`Organization`、`FAQPage`（英文，匹配 Googlebot 渲染，可拿 FAQ 富结果）三段 JSON-LD。
...
- **`public/`**：`robots.txt`、`sitemap.xml`、`ads.txt`（含占位说明）、`site.webmanifest`，以及 `about.html` / `privacy.html` / `contact.html` 三个独立可爬取的内容页（AdSense 审核基本必备，尤其隐私政策）。
