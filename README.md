...
`server/gen-repo-pages.js` 在 prerender 之后运行（`build` 脚本第三段）：用 Supabase anon 读取**全部** `daily / 全语言` 快照，按仓库聚合出「每天的排名 / star / 涨星」时间序列，为**上榜 ≥2 天**的仓库各生成一个自包含静态页 `dist/repo/<owner>/<name>/index.html`（干净 URL `/repo/owner/name`）。每页含：数据化导语、**内联 SVG 排名曲线**（rank 1 在顶，无需 JS）、完整上榜日志表、同语言交叉内链、`BreadcrumbList` JSON-LD。这是 `github.com/trending` 给不了的内容，也是成百上千个可索引 URL。

同时生成 `/repo` 列表中心页（按上榜天数排序，取前 200）和 `dist/sitemap-repos.xml`；`public/robots.txt` 已加第二条 `Sitemap:` 指向它。内链入口：首页 prerender 静态内容、React 页脚、各内容页 nav 都有「All repos」→ `/repo`，爬虫从中心页只会走到**确实存在**的页面（不会 404）。

常量在脚本顶部：`MIN_DAYS=2`（防薄内容/doorway 惩罚）、`MAX_PAGES=8000`（Cloudflare Pages 单次部署约 2 万文件上限，留足余量）、`HUB_LIMIT=200`。同样**绝不让构建失败**：无 env 或抓取失败则警告 + exit 0。本地可用 `node --env-file=.env server/gen-repo-pages.js` 单独验证。
